...
def hex_to_rgb(hex_rep):
    ...
    return sRGBColor(*[int(hex_rep[i + 1:i + 3], 16) for i in (0, 2 ,4)], is_upscaled=True)
...
def create_color_palette(start_rgb, end_rgb, num_colors, colorspace):
    ...
    points_between = list(zip(*[np.linspace(start=start_tuple[i], stop=end_tuple[i], num=num_colors) for i in range(3)]))

    # Convert points to RGB and then to hexes for the output
    rgb_colors = [convert_color(colorspace(*point), sRGBColor) for point in points_between]
...
def gen_random_colors(num_groups, colors=None):
    ...
    if len(colors) < num_groups:
        ...
        if type(new_colors[0][0]) == float:
            ...
            new_colors = [mpl.colors.to_hex([c[0], c[1], c[2]]).upper() for c in new_colors]
...
def gen_parl_points(seat_counts, 
    ...
    df_seat_lctns = pd.DataFrame(columns=['group', 'row', 'row_position', 'x_loc', 'y_loc'])
    ...
    if style == 'semicircle':
        ...
        extra_seat = total_seats - sum(row_seats) # 0 or 1 based on whether the seats divide evenly into the rows
        ...
        seats_shift = [i*int(num_rows/2) for i in seats_shift] # greater shift for higher rows for more equal spacing
        ...
        while total_seats > 0:
            ...
            if row_position_indexes[row_index] != []:
                index_to_assign = [i for i in df_seat_lctns.index \
                                   if df_seat_lctns.loc[i, 'row'] == row_index \
                                   and df_seat_lctns.loc[i, 'row_position'] == row_position_indexes[row_index][0]][0]
        ...
        y_coordinates = [i if (equa_distant_indexes.index(i) < int(len(equa_distant_indexes)/2) and len(equa_distant_indexes) % 2 == 0) 
                         or (equa_distant_indexes.index(i) < int(len(equa_distant_indexes)/2)+1 and len(equa_distant_indexes) % 2 != 0) \
                         else i+2 for i in equa_distant_indexes]
        ...
        if num_rows == 1:
            ...
            df_seat_lctns.sort_values(['row', 'x_loc', 'y_loc'], ascending=[True, True, True], inplace=True)
            ...
            while total_top_seats > 0:
                index_to_assign = [i for i in df_seat_lctns.index \
                                   ...
                                   and df_seat_lctns.loc[i, 'y_loc'] == top_y][0]
            ...
            while len([i for i in df_seat_lctns.index \
                       if df_seat_lctns.loc[i, 'x_loc'] == bottom_x \
                       and df_seat_lctns.loc[i, 'y_loc'] == bottom_y]) == 0:
                # Move down
                bottom_y -= 2
    ...
    if speaker:
        ...
        if style == 'semicircle':
            ...
            if len(y_coordinates) % 2 == 0:
                ...
                y_coordinate = (y_coordinates[int(middle_index_1)] + y_coordinates[int(middle_index_2)]) / 2
...
def swap_parl_allocations(df, row_1, pos_1, row_2, pos_2):
    ...
    """
    allocation_1 = df[(df['row'] == row_1) & (df['row_position'] == pos_1)]['group'].values[0]
    index_1 = df[(df['row'] == row_1) & (df['row_position'] == pos_1)].index
    
    allocation_2 = df[(df['row'] == row_2) & (df['row_position'] == pos_2)]['group'].values[0]
