...
def bar(counts, 
    ...
    if faction_names:
        factions_for_names = [[faction_names[i]]*len(counts[i]) for i in range(len(faction_names))]
        df_plot['faction'] = [item for sublist in factions_for_names for item in sublist]
    ...
    if horizontal:
        if stacked:
            if list not in [type(i) for i in counts]:
                for i in df_plot.index:
                    ax = sns.barplot(data=pd.DataFrame(df_plot.loc[i]).T, x='counts', y='group', 
                                     color=colors[i], saturation=dsat, 
                                     left=bar_starts[i], orient='h', ax=axis)

            else:
                pivot_plot = df_plot.pivot(columns='group', index='faction', values='counts').fillna(0).reindex(faction_names)
            ...
            if label_bars:
                if list not in [type(i) for i in counts]:
                    ...
                    for i in range(len(counts)):
                        ...
                        ax.text(x = label_position,
                                y = faction_start_idxs[i] + ax.patches[0].get_height() / 2, # all have equal height
                                s = label_text,
                                ha = "center")
            ...
            if list not in [type(i) for i in counts]:
                ...
                ax = sns.barplot(data=df_plot, x='counts', y='group', saturation=1, 
                                 left=0, orient='h', ax=axis)
            
            else:
                flat_counts = [item for sublist in counts for item in sublist]
                
                bar_positions = [0.8*i-0.4 for i in list(range(0, len(flat_counts)))] # 0.8 is the default width of plt.bar
                ...
                bar_locations = [bar_positions[i] + flat_bar_shifts[i] for i in range(len(bar_positions))]

                scaled_colors = [utils.scale_saturation(rgb=utils.hex_to_rgb(c), sat=dsat) for c in colors]
                ...
                factioned_bar_locations = utils.gen_list_of_lists(bar_locations, [len(f) for f in counts])
        ...
        if stacked:
            if list not in [type(i) for i in counts]:
                for i in df_plot.index:
                    ax = sns.barplot(data=pd.DataFrame(df_plot.loc[i]).T, x='group', y='counts', 
                                     color=colors[i], saturation=dsat, 
                                     bottom=bar_starts[i], orient='v', ax=axis)
            ...
            if label_bars:
                if list not in [type(i) for i in counts]:
                    ...
                    ax.text(x = ax.patches[0].get_x() + ax.patches[0].get_width() / 2.0,
                            y = label_position,
                            s = label_text,
                            ha = "center")
                    ...
                    for i in range(len(counts)):
                        ...
                        ax.text(x = faction_start_idxs[i] + ax.patches[0].get_width() / 2, # all have equal width
                                y = label_position,
                                s = label_text,
                                ha = "center")
            ...
            if list not in [type(i) for i in counts]:
                colors = [utils.scale_saturation(rgb=utils.hex_to_rgb(c), sat=dsat) for c in colors]
                sns.set_palette(colors)
                ax = sns.barplot(data=df_plot, x='group', y='counts', saturation=1, 
                                 bottom=0, orient='v', ax=axis)
    ...
    if (stacked and list not in [type(i) for i in counts]) or (not names and not faction_names):
        if horizontal:
