...
def semipie(counts, 
    ...
    """
    if colors:
        assert len(colors) == len(counts), "The number of colors provided doesn't match the number of counts to be displayed"

    elif colors == None:
        sns.set_palette("deep") # default sns palette
        colors = [utils.rgb_to_hex(c) for c in sns.color_palette(n_colors=len(counts), desat=1)]
    ...
    for i in range(len(counts)):
        wedge = mpatches.Wedge(center=(0,0), r=1, 
                               theta1=thetas[i+1], theta2=thetas[i],
                               facecolor=colors[i], 
                               width=donut_ratio)
