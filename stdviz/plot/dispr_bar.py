...
def dispr_bar(shares, 
    ...
    """
    assert len(shares) == len(allocations), "The number of different shares must equal the number of different seat allocations."
    ...
    disproportionality = [round(seat_percents[i] - share_percents[i], 4) for i in range(len(share_percents))]
    
    if percent == True:
        disproportionality = [round(disproportionality[i] / share_percents[i] * 100, 4) for i in range(len(share_percents))]
    ...
    if colors:
        assert len(colors) == len(shares), "The number of colors provided doesn't match the number of counts to be displayed"
        ...
        colors = [utils.rgb_to_hex(c) for c in sns.color_palette(n_colors=len(shares), desat=1)]

    ax = sns.barplot(data=df, x=df.index, y='disproportionality',
                     saturation=dsat, ax=axis)
    ...
    for p in ax.patches:
        ...
        if height < 0: # compensates for text height for negative bar labels
            ax.text(x = p.get_x() + p.get_width() / 2.0,
                    y = height + 1.5 * min([abs(i) for i in disproportionality]) * -1, # put below
                    s = str(height),
                    ha = "center")
        else:
            ax.text(x = p.get_x() + p.get_width() / 2.0,
